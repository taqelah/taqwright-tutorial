...
| --- | --- | --- | --- |
| `android-single` | one pinned emulator (`udid`) | `npx taqwright test --project android-single` | ⚠️ edit the AVD name/serial first |
| `android-pool-2` | fixed `pool` of 2, `workers: 2` | `… --project android-pool-2` | ⚠️ edit both pool entries first |
...
> ⚠️ **The AVD names in `taqwright.config.ts` are the author's, not yours.**
> `Pixel_10_Pro_XL` / `Pixel_10_Pro_XL_2` and the serials `emulator-5554` / `emulator-5556` are
> whatever happened to exist on the machine that wrote this lab — **your emulators are named
> something else.** Before running `android-single` or `android-pool-2`, run:
>
> ```bash
> npx taqwright devices     # lists  avd:<id>  + serial + state
> ```
>
> and paste **your** `avd:<id>` into `device.name` and **your** serial into `device.udid` /
> the `pool` entries. `android-pool-2` needs **two distinct AVDs** — the same AVD can't be
> listed twice. Don't want to edit anything? Use the `auto` projects.

**Start here** (run-anywhere, no config edits): `android-auto-1` (serial baseline) → `android-auto-2` (2-wide).
