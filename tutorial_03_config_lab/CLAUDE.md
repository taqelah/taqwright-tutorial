...
(`1_tests_and_timing/` … `6_appium_server/`) is a full, runnable taqwright project that runs the
...
- **No root project here** — there's no `package.json`/`node_modules` at `tutorial_03_config_lab/` itself.
  Work inside a sub-folder (`cd 4_artifacts && npm install && npm test`).
...
```bash
cd <n>-<concept>           # e.g. 2_execution_and_output
