...
// Runs under the `android` project (testDir: ./tests/android).
...
test('a valid user can log in (Android)', async ({ mobile }) => {
...
test('a wrong password stays on the login screen (Android)', async ({ mobile }) => {
  await usernameField(mobile).fill(USERNAME);
